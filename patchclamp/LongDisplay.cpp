...
#if USE_SOUND
    SealingSound=new Phonon::MediaObject(this);
    BreakingSound=new Phonon::MediaObject(this);
    QDir dir("");
    QString sounddir=dir.absolutePath()+QString("/../patchclamp/");
    SealingSound->setCurrentSource(Phonon::MediaSource(sounddir+"1.wav"));
    BreakingSound->setCurrentSource(Phonon::MediaSource(sounddir+"2.wav"));
    DisplayAudioOutput = new Phonon::AudioOutput(Phonon::AccessibilityCategory,this);
    phononpath = Phonon::createPath(SealingSound, DisplayAudioOutput);
#endif
...
int tLongDisplay::ScaleData(long graphtop, long graphbottom)
    ...
    for (int i=0;i<MAX_LONGVIEW_ENTRIES;i++)
        ...
        if (ProvisionalY<graphtop) ProvisionalY=graphtop; // too high,
               ...
               if (ProvisionalY >graphbottom) ProvisionalY=graphbottom;
...
int tLongDisplay::AddTwoSamples(double L, double H,long  Marker,long sound)
    ...
    if (PointsToPaint<MAX_LONGVIEW_ENTRIES)
        PointsToPaint++;
        // else return 2;
    if (L>H) { double T=L; L=H;H=T;}; // swap Lo and Hi values
    ...
    if ((d!=0)&&(sound))
        {
        if ((SealingSound->state()!=Phonon::PlayingState)&&
            (BreakingSound->state()!=Phonon::PlayingState))
            {
            if (d>0)
                {
               phononpath=Phonon::createPath(SealingSound,DisplayAudioOutput);
                ...
                {
                phononpath=Phonon::createPath(BreakingSound,DisplayAudioOutput);
                BreakingSound->play();
                };
            }
            else
...
int tLongDisplay::PaintAsLongTrace(QPainter * painter, QRect* rect)
    ...
    for (i=0;i<nGraphWidth;i++)
        ...
        if (bar_step>painter->font().pixelSize()+2)
        { bar_step=painter->font().pixelSize()+2; };
        ...
        for (long j=bandstart;j<bandend;j++)// j is real time index
            ...
            for (int m=0;m<Log.count();m++)
                ...
                if (long(floor(d))==j)
                   ...
                   painter->drawText(x,r->top()+band_top+band_height-((2+lastrandomposition)*PrintingFont.pixelSize()),
                                      Log.at(m).label);
                   long position=-1;
...
*/
            ...
            for (long l=0;l<8;l++)
                ...
                if ((solutions & (4<<l))!=0)
                    ...
                    if ((print_line))
                            ...
                            painter->drawText(textat,r->top()+band_top+(l+1)*bar_step-painter->font().pixelSize()/10,AgonistNames->at(l));
                            };
        ...
        for (int j=bandstart;j<bandend;j++)
            ...
            if ((j%60)==0)
                ...
                painter->drawText(x+timescaleheight/12,timescaletop+(timescaleheight*17)/20, QString("%1").arg(j/60));
                }
            else
                {
                if ((j%10)==0)
            ...
            if ((j%100)==0)
                {
                painter->drawRect(x,timescaletop+timescaleheight-timescaleheight/6,blockwidth,timescaleheight/6);

                }
            else
                {
                if ((j%50)==0)
                    painter->drawRect(x,timescaletop+timescaleheight-timescaleheight/12,blockwidth,timescaleheight/12);
                };
...
int tLongDisplay::Paint(QPainter * painter)
    ...
    switch (CurrentDisplayMode)
        {
        case eSealTest:
            {PaintAsSealTest(painter); break;};
        case eMembraneTest:
            {PaintAsMembraneTest(painter);break;};
        case eRecording:
            {PaintAsLongTrace(painter);break;};
        default:
            {
